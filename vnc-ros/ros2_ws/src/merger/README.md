...
**Subscriptions** `/<robot_id>/map` (one per robot)  `nav_msgs/msg/OccupancyGrid` Local SLAM map produced by each robot.
**Publications** `/merged_map` `nav_msgs/msg/OccupancyGrid` Latest globally fused map (frame = `odom_1`).
**Static TFs** `odom_<anchor> → odom_<follower>`  `tf2_ros/StaticTransformBroadcaster`  Allows planners / visualisers to transform poses between robot odom frames.

> The node creates a separate subscriber for every robot ID supplied on the command line (or via the launch file).

---

## ⚙️ Parameters (ROS 2 parameters)
